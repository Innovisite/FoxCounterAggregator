                ...
                function(
                    ...
                    function setEditorValue(user) {
                        if ($scope.jsonEditor !== undefined) {
                            if (user !== undefined && user.app_data !== undefined) {
                                $scope.jsonEditor.setValue(angular.toJson(user.app_data, true));
                            } else {
                    ...
                    function setDashboardValue(user) {
                        if ($scope.htmlEditor !== undefined) {
                            if (user !== undefined && user.app_data !== undefined && user.app_data.dashboard !== undefined) {
                                $scope.htmlEditor.setValue(user.app_data.dashboard);
                            } else {
                        ...
                        if ($scope.jsonEditor !== undefined && $scope.currentUser !== undefined) {
                            try {
                                if ($scope.jsonEditor.getValue() === "{}") {
                                    $scope.currentUser.app_data = undefined;
                                } else {
                                    jsonlint.parse($scope.jsonEditor.getValue());
                                    $scope.currentUser.app_data = angular.fromJson($scope.jsonEditor.getValue());
                                }
                                $scope.jsonValid = true;
                            } catch (err) {
                    ...
                    $scope.htmlChanged = function() {
                        if ($scope.htmlEditor !== undefined && $scope.currentUser !== undefined) {
                            if ($scope.currentUser.app_data !== undefined) {
                                $scope.currentUser.app_data.dashboard = $scope.htmlEditor.getValue();
                                $scope.htmlValid = true;
                            }
                        }
                    };
